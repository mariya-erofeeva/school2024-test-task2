...
## Описание реализации
Создана функция popularCategories, которая принимает на вход файл формата json. Для отбора категорий товаров, которые были куплены в предновогодний период (с 1 по 31 декабря) использовался метод strptime() из модуля datetime. Алгоритм осуществляет подсчет количества купленных товаров из каждой категории и выводит в алфавитном порядке категории с наибольшим количеством.

## Инструкция по сборке и запуску решения
Для работы функции необходимо, чтобы в папке с программой (report.py) находился файл "input.json". Решение протестировано в Python для Windows версии 3.9.
